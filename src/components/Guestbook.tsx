...
export default function Guestbook({ wishes, onAddWish, whatsappNumber, isAdmin = false }: GuestbookProps) {
    ...
    <div className="w-full max-w-4xl mx-auto my-14 px-4" id="congratulations-wall">
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="font-serif text-3xl md:text-4xl text-olive-medium font-bold mt-1">
          {isAdmin ? "Wishes Wall (Admin View)" : "Leave a Blessing for the Newlyweds"}
        </h2>
        ...
        {!isAdmin && (
          <p className="text-xs text-charcoal/60 mt-3 font-semibold">
            تهنئتكم ومشاعركم الطيبة تصل مباشرة للعروسين بكل خصوصية وسرية 🤍🕊️
          </p>
        )}
      </div>

      {isAdmin ? (
        /* Grid Layout: Form and Map of Wishes (visible to admin only) */
        ...
        /* Non-admin View: Only the submission form is displayed, beautifully centered with no list visible */
        <div className="max-w-xl mx-auto bg-ivory border border-gold-medium/20 rounded-3xl p-6 md:p-8 shadow-md">
          <div className="flex items-center gap-2 mb-4 border-b border-champagne pb-3">
            <MessageSquare className="h-5 w-5 text-gold-medium" />
            <span className="font-serif font-bold text-olive-medium text-sm tracking-widest uppercase">
              أرسل تهنئة خاصة وعائلية / Write a Blessing
            </span>
          </div>
          ...
          {isWishesSubmitted ? (
            <div className="py-8 text-center animate-fade-in flex flex-col items-center">
              <div className="h-12 w-12 bg-olive-light text-olive-medium rounded-full flex items-center justify-center mb-3">
                <Check className="h-6 w-6 animate-[scale-up_0.3s_ease]" />
              </div>
              <p className="font-serif text-sm font-semibold text-olive-dark">
                تم إرسال تهنئتكم بنجاح! 🌸✨
              </p>
              <p className="text-[11px] text-charcoal/75 mt-2 max-w-[280px] mb-6 leading-relaxed font-semibold">
                وصلت تمنياتكم الطيبة للعروسين أحمد ويمنى بكل محبة وستظل محفوظة كذكرى عطرة في لوحة التحكم الخاصة بهما.
              </p>
              ...
              <button
                ...
                className="px-6 py-2.5 bg-olive-medium hover:bg-olive-dark text-white rounded-xl text-xs font-semibold tracking-wider transition-all hover:scale-[1.03] active:scale-95 cursor-pointer shadow-sm"
              >
                اكتب تهنئة أخرى • Write Another Blessing
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              <div className="space-y-1">
                <label className="text-[10px] font-semibold text-gold-dark tracking-wider uppercase">الاسم الكريم / Your Name</label>
                <input
                  type="text"
                  required
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="مثال: أدهم هلالي / Adham Helaly"
                  className="w-full px-4 py-2.5 rounded-xl bg-ivory/50 border border-gold-medium/20 text-xs focus:ring-2 focus:ring-olive-medium focus:outline-none"
                />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-semibold text-gold-dark tracking-wider uppercase">الرسالة أو التهنئة / Congratulatory Message</label>
                <textarea
                  required
                  value={wishText}
                  onChange={(e) => setWishText(e.target.value)}
                  ...
                  rows={4}
                  className="w-full px-4 py-2.5 rounded-xl bg-ivory/50 border border-gold-medium/20 text-xs focus:ring-2 focus:ring-olive-medium focus:outline-none resize-none"
                />
              </div>

              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-olive-medium hover:bg-olive-dark text-white font-serif tracking-widest text-[11px] uppercase shadow-sm transition-all hover:scale-[1.02] cursor-pointer font-bold"
              >
                إرسال التهنئة للعروسين • Send My Blessing
              </button>
            </form>
          )}
        </div>
